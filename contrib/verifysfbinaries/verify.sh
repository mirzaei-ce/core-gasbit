...
###   This script attempts to download the signature file SHA256SUMS.asc from gasbit.org
...
WORKINGDIR="/tmp/gasbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://gasbit.org/bin/"
VERSIONPREFIX="gasbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'gasbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Gasbit Core binary release signing key installed?"
   fi
