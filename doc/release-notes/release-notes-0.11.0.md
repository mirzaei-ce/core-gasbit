Gasbit Core version 0.11.0 is now available from:

  <https://gasbit.org/bin/gasbit-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Gasbit Core or other software:
...
defaults to 0.00001. This will cause transactions with fewer GBT/kB fee to be
...
For example, add the following to `gasbit.conf`:
...
related to the blockchain in the gasbit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Gasbit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Gasbit Core will 
...
For an example script see [gasbit-submittx](https://github.com/laanwj/gasbit-submittx).
...
- #5334 `cf87536` libgasbitconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new gasbit-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only GasbitAmountField
- #5753 `0f44672` Add gasbit logo to about screen
...
- #5809 `46bfbe7` Add gasbit-cli man page
...
- #5849 `41259ca` contrib/init/gasbitd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Gasbit-Qt.app to Gasbit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for gasbit-tx)
