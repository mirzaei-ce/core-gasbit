...
namespace {
    const int MAX_OGBTOUND_CONNECTIONS = 8;
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Gasbit users should be well used to small numbers with many decimal places by now :)
...
static void AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    CAddress addr;
    int nInbound = 0;
    int nMaxInbound = nMaxConnections - MAX_OGBTOUND_CONNECTIONS;
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Gasbit " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Gasbit Core is probably already running."), addrBind.ToString());
        else
...
void StartNode(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if (semOutbound == NULL) {
        ...
        int nMaxOutbound = min(MAX_OGBTOUND_CONNECTIONS, nMaxConnections);
...
bool StopNode()
{
    ...
    MapPort(false);
    if (semOutbound)
        for (int i=0; i<MAX_OGBTOUND_CONNECTIONS; i++)
            semOutbound->post();
